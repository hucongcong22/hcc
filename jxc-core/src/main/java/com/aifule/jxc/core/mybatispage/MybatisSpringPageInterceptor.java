...
public class MybatisSpringPageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        if (invocation.getTarget() instanceof StatementHandler) { // 控制SQL和查询总数的地方
            log.debug("sql=====" + ((StatementHandler) invocation.getTarget()).getBoundSql().getSql());
...
//					page.setResults((List) resultObj);
    ...
    protected String buildMysqlPageSql(Page<?> page, String sql) {
        int start = page.getStart() - 1 < 0 ? 0 : page.getStart();
        int end = page.getEnd() < 1 ? 999999999 : page.getEnd() - start;
        return new StringBuilder(sql).append(" limit ").append(start).append(",").append(end < 1 ? 0 : end)
                .toString();
    }
