...
var  goodsWrite =new Vue({
    ...
    methods:{
        submit:function (){
            if (this.t1==2){
                var data=ajax("../api/goods/write","post",{"name":this.name,"units":this.units,"type":this.type},false).data
                if (data==true){
                    alert("商品添加成功")
        ...
        updata:function () {
            var data=ajax("../api/goods/updata","post",{"id":this.id,"name":this.name,"units":this.units,"type":this.type},false).data
            if (data==true){
                alert("商品修改成功")
                var datas=ajax("../api/goods/list","post",{},false)
                goods.goodsList =datas.data
