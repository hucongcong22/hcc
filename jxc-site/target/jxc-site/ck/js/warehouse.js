...
var  warehouseWrite =new Vue({
    ...
    methods:{
        submit:function (){
            if (this.t1==2){
                var data=ajax("../api/warehouse/write","post",{"name":this.name,"info":this.info,"address":this.address},false).data
                if (data==true){
                    alert("商品添加成功")
        ...
        updata:function () {
            var data=ajax("../api/warehouse/updata","post",{"id":this.id,"name":this.name,"info":this.info,"address":this.address},false).data
            if (data==true){
                alert("商品修改成功")
                var datas=ajax("../api/warehouse/list","post",{},false)
                warehouse.warehouseList =datas.data
